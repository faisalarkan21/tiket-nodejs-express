...
exports.index = function (req, res) {
    ...
    var query = connection.query("select * from periode", function (err, dataJadwal) {
        ...
        var query = connection.query("select COUNT(*) AS tiketCount FROM pembeli", function (err, countKuota) {
            ...
            let isFull = counting > 0
                ? htmlCountAvailable
                : htmlCountFull;
            let isFullLokasi = counting > 0
                ? counting
                : 'Penuh';
            let isFullDisable = counting > 0
                ? false
                : true;
...
exports.login = function (req, res, next) {
    ...
    var query = connection.query("select COUNT(*) AS tiketCount FROM pembeli", function (err, countKuota) {
        ...
        let htmlCountFull = '<label class="judul3 size" for="">Status Kuota Penuh <i style="padding-left:8px;' +
        ...
        if (!req.session.namaAdmin && req.session.namaSession !== undefined) {
            ...
            res.render('user/login', {isDisable: isFullDisable});
...
exports.membuktikan = function (req, res, next) {
    ...
    var query = connection.query("select * from pembeli where email_pembeli = ? ", req.body.email, function (err, data) {
        ...
        // //disini if (!data) {     console.log("kaga sama");
        // res.render('user/login',{error: "has-error is-empty", data:"<label
        // class='control-label' id='error' >Data anda tidak ada didalam
        // database</label>"}); } else if (data.admin == false) {
        // req.session.namaSession = data.email_pembeli;     req.session.admin =
        // data.admin;     console.log(req.session);     res.send("profile"); } else if
        // (data.admin == true) {     console.log("disini admin");
        // req.session.namaSession = data.name;     req.session.admin = data.admin;
        // //test     console.log(req.session);     res.redirect('admin/dashboard'); }
...
exports.profile = function (req, res) {
    ...
    var query = connection.query("select * from pembeli where email_pembeli = ? ", req.session.namaSession, function (err, data) {
        ...
        let status = data[0].email_verification === 1
            ? ''
...
exports.validasi = function (req, res) {
    ...
    // case looooopp hati2 console.log("hereeeeeeee broh");
...
exports.validasiPost = function (req, res) {
    ...
    // Masuk Pembeli Utama console.log(updatePembeliUtama);

    var query = connection.query("update pembeli set ? where id_pembeli =  ?", [
        updateUser, req.session.nomor_pembeli
    ], function (err, rows) {
    ...
    var query = connection.query("select * from kursus where id_kursus=?", req.params.id, function (err, kursus) {
        ...
        var query = connection.query("select * from trainer where id_kursus=?", req.params.id, function (err, trainer) {
            ...
            var query = connection.query("select * from periode", function (err, dataJadwal) {
                ...
                // var stringDate = jadwal[0].tgl_mulai.toString(); var stringDate2 =
                // jadwal[0].tgl_selesai.toString(); const ReBegin = stringDate.slice(3, 15);
...
exports.adminValidasi = function (req, res) {
    ...
    var query = connection.query('select * from admin where email_admin = ?  ', req.body.email, function (err, data) {
        ...
        if (data.length < 1) {
            ...
            var bytes = CryptoJS
                .AES
                .decrypt(data[0].pass_admin, 'aa38bf3e39f6f9d51c84b02d583eb7ca57e5a1b4ed22b54380c77b9e45f4671a');
            var decryptedData = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
            console.log(decryptedData);
...
exports.adminDashboard = function (req, res) {
    ...
    var query = connection.query("select COUNT(*) AS tiketCount FROM pembeli INNER JOIN detil_pesan_tiket on pembe" +
            "li.id_pembeli=detil_pesan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_" +
            "pesan_tiket.id_pembeli=pembeli_validasi.id_pembeli",
    function (err, jumlahTiketTerjual) {

        // console.log(jumlahTiketTerjual[0].hitung);

        var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
                "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
                "i=pembeli_validasi.id_pembeli ",
        function (err, pembeliWaiting) {

            var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
                    "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
                    "i=pembeli_validasi.id_pembeli WHERE detil_pesan_tiket.uang_transfer = 500000 ",
            function (err, Lunas) {

                var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
                        "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
                        "i=pembeli_validasi.id_pembeli WHERE detil_pesan_tiket.uang_transfer = 0 ",
                function (err, pembeliBelumLunas) {
                    ...
                    var query = connection.query("SELECT SUM(detil_pesan_tiket.uang_transfer) AS totaltf FROM detil_pesan_tiket ", function (err, totalPendapatan) {
                        ...
                        var query = connection.query("SELECT SUM(detil_pesan_tiket.harga_tiket) AS totalTiket FROM detil_pesan_tiket ", function (err, totalEstimasiPembeli) {
                            ...
                            var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
                                    "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
                                    "i=pembeli_validasi.id_pembeli WHERE pembeli_validasi.uang_transfer_validasi = 50" +
                                    "0000 ",
                            function (err, validasiTiket) {

                                var query = connection.query("SELECT  * FROM  periode WHERE  tgl_daftar_mulai >= '2017-08-09' AND tgl_selesai " +
                                        " <= '2017-10-14'",
                                function (err, jadwal) {
                                    ...
                                    // var waitingTiket = 0; waitingTiket =    pembeliWaiting[0].waitingCount;
...
exports.semuaUser = function (req, res) {
    ...
    var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
            "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
            "i=pembeli_validasi.id_pembeli limit 60",
    function (err, pembeliSemua) {
...
exports.userLunas = function (req, res) {
    ...
    var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
            "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
            "i=pembeli_validasi.id_pembeli WHERE detil_pesan_tiket.uang_transfer = 500000 lim" +
            "it 60",
    function (err, Lunas) {
...
exports.userBelumLunas = function (req, res) {
    ...
    var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
            "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
            "i=pembeli_validasi.id_pembeli WHERE detil_pesan_tiket.uang_transfer = 0 limit 60",
    function (err, pembeliBelumLunas) {
...
exports.waitingList = function (req, res) {
    ...
    var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
            "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
            "i=pembeli_validasi.id_pembeli limit 60, 18446744073709551615",
    function (err, pembeliWaiting) {

        // console.log(pembeliWaiting < 1);

        var pembeliFilterAda,
            pembeliFilterNull;
...
exports.userDetail = function (req, res) {
    ...
    var query = connection.query("select * FROM pembeli INNER JOIN detil_pesan_tiket on pembeli.id_pembeli=detil_p" +
            "esan_tiket.id_pembeli INNER JOIN pembeli_validasi on detil_pesan_tiket.id_pembel" +
            "i=pembeli_validasi.id_pembeli WHERE pembeli.id_pembeli = ?",
    req.params.id, function (err, userDetail) {
...
exports.tiketPost = function (req, res) {
    ...
    var query = connection.query("update pembeli_validasi set ?  where id_pembeli =  ?", [
        updateValidasi, req.session.nomor_pembeli
    ], function (err, data) {
        if (err) {
            console.log(err);
            return next("Mysql error, check your query");
            ...
            var query = connection.query("select count(*) as total FROM trainer WHERE id_kursus = ?", 2, function (err, getKs2) {
                ...
                var query = connection.query("select count(*) as total FROM trainer WHERE id_kursus = ?", 3, function (err, getKs3) {
                    ...
                    var query = connection.query("select count(*) as total FROM trainer WHERE id_kursus = ?", 4, function (err, getKs4) {
                        ...
                        let trainerJadwal = trainerDetail[0].id_kursus == null
                            ? "Belum Ditentukan"
                            : "Minggu Ke - " + trainerDetail[0].id_kursus;
...
exports.trainerDetailUpdate = function (req, res) {
    ...
    var query = connection.query("select count(*) as total FROM trainer WHERE id_kursus = ?", req.body.setJadwal, function (err, getCountKs) {
        ...
        if (getCountKs[0].total === 3) {
            ...
            res.redirect('/admin/dashboard/trainer/?idTrainerError=' + req.params.id + '&messageError=<label style="font-size:14px; margin-bottom:10px;  margin-top:10px' +
                    '; color:red !important;">Jadwal minggu ke - ' + req.body.setJadwal + ' sudah terisi penuh.</label>');

        } else {

            var query = connection.query("update trainer set ? where trainer_id =  ?", [
                updateJadwal, req.params.id
            ], function (err, rows) {
...
exports.verification = function (req, res) {
    ...
    var query = connection.query("select * from pembeli where token_verification = ? ", req.query.token, function (err, data) {
        ...
        var query = connection.query("update pembeli set ? where token_verification =  ?", [
            updateVerification, req.query.token
        ], function (err, rows) {
...
exports.userDetailTest = function (req, res) {
    ...
    req.params.id, function (err, userDetail) {
        ...
        res.json({userDetail: userDetail[0]});
        // console.log(user_gold);
